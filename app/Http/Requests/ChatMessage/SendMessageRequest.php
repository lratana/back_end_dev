...
class SendMessageRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        if ($this->type === 'text') {
            ...
            $rules['content'] = ['required', 'file', 'max:51200']; // 50MB max for VPS

            if ($this->type === 'image') {
                $rules['content'][] = 'mimes:jpg,jpeg,png,gif,webp';
            } elseif ($this->type === 'video') {
                // Accept common video extensions and explicit video MIME types (helps when clients send differing MIME headers)
                $rules['content'][] = 'mimes:mp4,mov,avi,webm,mpg,mpeg,3gp';
                $rules['content'][] = 'mimetypes:video/mp4,video/quicktime,video/x-msvideo,video/webm,video/mpeg,video/3gpp';
    ...
    public function messages(): array
    {
        return [
            ...
            'content.max' => 'File size must not exceed 50MB',
